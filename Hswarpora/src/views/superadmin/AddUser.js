     ...
     if (Object.keys(validationErrors).length === 0)
     {
       ...
       CurrentData.sid=1;
        ...
        try {
            ...
            const response = await axios.post('https://createuser-324cqhxlga-uc.a.run.app', { UserEmail: lowerCaseEmail, password:CurrentData.password, UserName: lowerCaseName });
            console.log("response---->",response)
            console.log("response---->",dataTobesend)
            let uid = response.data.data.uid; // Get the generated uid
            dataTobesend['uid'] = uid;
            dataTobesend['tid'] = uid;
            let CreateRes=await firestoreQueries.updateOrCreateById(DatabaseName, "users",  uid, dataTobesend);
             console.log("CreateRes---->",CreateRes)
             TooltipsPopovers(CreateRes.status,CreateRes.message,CreateRes.status)
            setCurrentData({});
            hideLoading()
          } catch (error) {
          console.log("error---->",error)
            ...
            {
              TooltipsPopovers(error.response.data.status,error.response.data.data,error.response.data.status)
            }
