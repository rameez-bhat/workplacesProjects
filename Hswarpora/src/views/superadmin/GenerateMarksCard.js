...
setCurrentData((prevValues) => 
    ...
    //const ListOfTeachers=await firestoreQueries.FetchDataFromCollection(DatabaseName, "users", 1000, 'role', '==', "Teacher","name","asc");
     ...
     ListClassOption1 = [
      ...
      ListOfSubjectFullObject[classes.class]=classes?.[classes.currentyear]?.['teachers'] ? {"TeacherList":classes?.[YearToBeSelected]?.['teachers'],"SubjectList":classes?.subjects} :{"TeacherList":{},"SubjectList":classes?.subjects};
  ...
  const fetchData = async () => {
   //await firestoreQueries.copyCollection(DatabaseName,"studentsBK","students");
    ...
    generateQRCode();
     ...
     ListClassOption1 = [
      ...ListOfClasses.map(classes => {
      ListOfSubjectFullObject[classes.class]=classes?.[YearToBeSelected]?.['teachers'] ? {"TeacherList":classes?.[YearToBeSelected]?.['teachers'],"SubjectList":classes?.subjects} :{"TeacherList":{},"SubjectList":classes?.subjects};
      FullClassDetails[classes.class]=classes;
      return{ value: classes.class, label: classes.class };
...
{
  //const subjects = ["english", "math", "urdu", "kashmiri", "science", "s_science", "co-curricular-activities"];
  const subjectNames = {
    english: "English",
    math: "Math",
    urdu: "Urdu",
...
console.log("setListAllStudentWithDetails--->",ListAllStudentWithDetails)
console.log("FullClassDetails--->",FullClassDetails)
console.log("CurrentData--->",CurrentData)
console.log("YearToBeSelected--->",YearToBeSelected)


const SubjectList=ListAllStudentWithDetails[0][YearToBeSelected]['subjects'];
console.log("SubjectList--->",SubjectList)
  const examKeys = Object.keys(
    FullClassDetails?.[CurrentData.class.label]?.[YearToBeSelected]?.["exams"] || {}
  ).sort();
console.log("examKeys--->",examKeys)
console.log("Array(examKeys.length).fill()---->",Array(examKeys.length).fill(""))
const studentHeaders = ["Name", "Fathers Name", "D-O-B", "Class", "Roll No"];
const mergeRanges = [];
const subExamHeaders = ["","","","",""];
let currentCol = studentHeaders.length;
const cleanedExamKeys = examKeys.map(key => key.replace("F7$$", ""));
const subjects = [
...
subjects.forEach(({ key, label }) => {
  if (SubjectList?.[key]) {
  const Labell=label.replace("F7$$", "");
    studentHeaders.push(Labell, ...Array(examKeys.length).fill(""));
    
	subExamHeaders.push(...cleanedExamKeys,"Total");
    // Push merge range: merge from currentCol to currentCol + examKeys.length
    mergeRanges.push({
      from: currentCol + 1,
      to: currentCol + examKeys.length,
      Labell,
    });
	
    currentCol += 1 + examKeys.length;
  }
});
studentHeaders.push("");
subExamHeaders.push("Grand Total");
const workbook = new ExcelJS.Workbook();
const worksheet = workbook.addWorksheet('Result Sheet');
const headerRow1 = worksheet.addRow(studentHeaders);
const headerRow2 = worksheet.addRow([...Array(studentHeaders.length - subExamHeaders.length).fill(""), ...subExamHeaders]);
const RowList=[];
ListAllStudentWithDetails.forEach(( Details,key ) => {
const DetailsD=Details[YearToBeSelected]
const DBT=dayjs(new Date(Details['dobActual'].seconds * 1000)).format(dateFormat)
const originalRollNo = DetailsD['particulars']['rollno'];
const modifiedRollNo = originalRollNo?.toString().slice(1);
const ListDet=[Details['name'],Details['fathersname'],DBT,DetailsD['particulars']['currentclass'],modifiedRollNo];
...
/*subjects.forEach(({ key, label }) => 
{
	if(DetailsD['subjects']?.[key])
	{
		//const valuesInOrder = Object.keys(DetailsD['subjects']?.[key]).map(keyB => DetailsD['subjects']?.[key][keyB]);
		console.log("======>",DetailsD['subjects']?.[key])
		 const valuesInOrder = Object.keys(DetailsD['subjects'][key])
    .map(k => parseFloat(DetailsD['subjects'][key][k]) || 0);
    console.log("valuesInOrder======>",valuesInOrder)
		const total = valuesInOrder.reduce((sum, val) => sum + val, 0);
		GTotal=GTotal+total;
		ListDet.push(...valuesInOrder,total);
	}
	
})*/
subjects.forEach(({ key, label }) => {
console.log("key====>",key)
console.log("DetailsD['subjects']?.[key]====>",DetailsD['subjects']?.[key])
  if (DetailsD['subjects']?.[key]) {
    // Maintain order as per the keys inside the object
    const keysInOrder = Object.keys(DetailsD['subjects'][key]).sort((a, b) => 
      a.localeCompare(b, undefined, { numeric: true }) // ensures numeric keys are ordered properly
    );
    let valuesInOrder=[];
	if((key=="retail" || key=="apparel") && keysInOrder.length==0)
	{
		valuesInOrder = [0,0,0];
	}
	else
	{
		valuesInOrder = keysInOrder.map(k => parseFloat(DetailsD['subjects'][key][k]) || 0);
	}
...
console.log("valuesInOrder====>",valuesInOrder)
    ...
    {
  		ListDet[IndexSet]=value;
  		IndexSet++;
	});
    //ListDet.push(...valuesInOrder, total);
    ListDet[IndexSet]=total;
	IndexSet++;
...
while (ListDet.length < subExamHeaders.length-1) {
  ListDet.push('0');
}
console.log("ListDet===>",ListDet)
ListDet.push(GTotal);
RowList.push(ListDet);
const row = worksheet.addRow(ListDet);
row.eachCell((cell) => {
...
mergeRanges.forEach(({ from, to, label }) => {
...
//studentData.forEach(row => worksheet.addRow(row));
...
workbook.xlsx.writeBuffer().then((buffer) => {
  saveAs(new Blob([buffer]), 'AllStudents_ResultSheet1.xlsx');
});
...
let GrandMarksObtained=0;
    ...
    doc.rect(1, 2, 208, 28); // Header border
	const session = `${YearToBeSelected - 1}-${String(YearToBeSelected).slice(2)}`;
	...
	const sessionText = `Session: ${session}`;
 	const textWidth = doc.getTextWidth(sessionText);
 	const xCenter = (pageWidth - textWidth) / 2; // center horizontally

doc.text(sessionText, xCenter, 45);
    doc.setFont("helvetica", "normal");
    doc.setTextColor("#8C183B");
    doc.text("Name: ", 7, 60);
    doc.setFont("helvetica", "bold"); // Bold font
    doc.setTextColor("#000000");
    doc.text(`${currentResult?.name}`, doc.getTextWidth("Name: ") + 7, 60);
    //doc.text(`Roll Number: ${String(currentResult?.rollno)?.slice(1)}`, 105, 60, { align: "left" });
    doc.setFont("helvetica", "normal");
    doc.setTextColor("#8C183B");
    doc.text("Roll Number: ", 110, 60);
    doc.setFont("helvetica", "bold"); // Bold font
    doc.setTextColor("#000000");
    doc.text(`${String(currentResult?.particulars?.rollno)?.slice(1)}`, doc.getTextWidth("Roll Number: ") + 110, 60);
    //doc.text(`Father's Name': ${currentResult?.fathersname}`, 15, 70);
    doc.setFont("helvetica", "normal");
    doc.setTextColor("#8C183B");
    doc.text("Father's Name: ", 7, 70);
    doc.setFont("helvetica", "bold"); 
    doc.setTextColor("#000000");
    doc.text(`${currentResult?.fathersname}`, doc.getTextWidth("Father's Name: ") + 7, 70);
    //doc.text(`D.O.B: ${dayjs(new Date(currentResult?.dobActual.seconds * 1000)).format(dateFormat)}`, 105, 70, { align: "left" });
    doc.setFont("helvetica", "normal");
    doc.setTextColor("#8C183B");
    doc.text("D.O.B: ", 110, 70);
    doc.setFont("helvetica", "bold"); 
    doc.setTextColor("#000000");
    doc.text(`${dayjs(new Date(currentResult?.dobActual.seconds * 1000)).format(dateFormat)}`, doc.getTextWidth("D.O.B: ") + 110, 70);
    //doc.text(`Mother's Name': ${currentResult?.mothersname}`, 15, 80);
    doc.setFont("helvetica", "normal");
    doc.setTextColor("#8C183B");
    doc.text("Mother's Name: ", 7, 80);
    doc.setFont("helvetica", "bold"); 
    doc.setTextColor("#000000");
    doc.text(`${currentResult?.mothersname}`, doc.getTextWidth("Mother's Name: ") + 7, 80);
    //doc.text(`Admission No: ${currentResult?.admissionno}`, 105, 80, { align: "left" });
    doc.setFont("helvetica", "normal");
    doc.setTextColor("#8C183B");
    doc.text("Admission No: ", 110, 80);
    doc.setFont("helvetica", "bold"); 
    doc.setTextColor("#000000");
    doc.text(`${currentResult?.admissionno}`, doc.getTextWidth("Admission No: ") + 110, 80);
    ...
    doc.setFont("helvetica", "bold"); 
    doc.setTextColor("#000000");
    doc.text(`${CurrentData.class?.label}`, doc.getTextWidth("Class: ") + 7, 90);
  ...
  else
  {
    ...
    BodyRowGMain.push([{ content: "Grand Total", colSpan: HeadRowsG.length-2, styles: { halign: "center", fontStyle: "bold" } },GrandMaxMarks,GrandMarksObtained]);
    BodyRowGMain.push([{ content: "Performance", colSpan: HeadRowsG.length-2, styles: { halign: "center", fontStyle: "bold" } },{ content: GreadS, colSpan: 2, styles: { halign: "center", fontStyle: "bold" } }]);   
      ...
      />
      	 {errors.subject  && <span class="validationerror">{errors.subject }</span>}
                ...
                <CCol xs={6}>
                  ...
                  <CCol xs={6}>
                  <CButton color="primary" type="button"
                   onClick={(event) => generateExcelForAllStudents()}
                   >
