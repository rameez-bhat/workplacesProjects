...
fetchData();
  ...
  //await firestoreQueries.updateFieldForAllDocuments(DatabaseName,"users","userType","district");
    let fetchName="";
  if(getActUser.name)
  {
    ...
    {
      DDOLIST=await firestoreQueries.FetchDataFromCollection(DatabaseName, "users", 1000, "zone", "==", getActUser.zone,"zone","asc","ddocode","asc");
    }
    else
    {
      DDOLIST=await firestoreQueries.FetchDataFromCollection(DatabaseName, "users", 1000, "role", "==", "DDO","zone","asc","ddocode","asc");
    }
    if(DDOLIST.length)
    {
  ...
  <a href={`/ddo/adddata/${settingtype}/${item.id}`} target="_blank">
    {item.ddocode}
  </a>
</CTableHeaderCell>
